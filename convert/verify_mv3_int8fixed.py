...
# Match the DEPLOYED INT8 espdl: apply the same exponent-overflow surgery the
# quantize_mv3_int8_fixed.py export does, so the executor simulates exactly what
# the device runs (conv2d_13 gets coarsened -27 -> -8, etc.).
if os.environ.get("FIX_EXP") == "1":
    def _exp(scale):
    ...
    for op in quantized.operations.values():
        ...
        if out_e < need:
            ...
            print(f"  fixed {op.name}: out_exp {out_e} -> {need}")
            fixed += 1
    print(f"exponent overflow fixes applied: {fixed}")
